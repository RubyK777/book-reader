...
- Phase 2 ships the **four-tab shell**: Saved/Review/Settings show only their UX_SPEC §2 empty states (Saved: "Words you save while reading appear here"; Review: the never-saved cross-promo; Settings: a minimal `Form` hosting the **native-language** picker (`@AppStorage("nativeLanguage")`, the user's own/translation-destination language — DECISIONS #25) plus speech rate). Rejected hiding tabs until Phase 3: adding tabs later shifts muscle memory, and the empty states teach the save→review loop from day one.
...
- **Create**: `[+]` → sheet `BookFormView(mode: .create)` — title field, optional cover via `PhotosPicker`. **No forced source-language pick at create** (CHANGE 1): `Book.languageCode` is auto-set from the confirmed source language of the first page in OCRReview (§5), so a book created here starts with an empty/`nil`-ish placeholder language shown as "Set on first scan" and no `Picker` requirement. A translate-to `Picker` (optional, seeded from the **native language** when translation is on, includes "None") sets `Book.translationLanguage`; the destination is the user's native language, detail in [TRANSLATION_DESIGN.md](TRANSLATION_DESIGN.md) §7. Save inserts `Book` into `modelContext`.
- **Rename / cover / language**: swipe action or context menu → same `BookFormView(mode: .edit(Book))`. In edit mode the source-language `Picker` is **shown and editable**, listing the **full `LanguageCatalog` set** (Vision's recognizable languages, not a curated nine — DECISIONS #25) and prefilled from the detected/confirmed code, with the caption "Changing this only affects the voice — it does not re-OCR existing pages"; the translate-to `Picker` is also editable (changing it clears this book's cached `Sentence.translatedText` per [TRANSLATION_DESIGN.md](TRANSLATION_DESIGN.md)). Trade-off vs the old hard lock: language is now auto-detected, so a rare correction is cheaper to allow than to block; re-OCR-on-language-change is still not a v1 feature.
...
**DECIDED: the ephemeral quick-scan path is retired.** Every successful scan persists. Justification: (a) plan §8 already decided "keep pages forever"; (b) two Reader data paths means every Phase 2/3 feature (stars, save-word, SRS) needs an "unless ephemeral" branch; (c) "I just wanted to try it" is served by quick-create + swipe-to-delete, which costs the user two taps, not the codebase a second mode. `ScanHomeView` is deleted; its **mandatory language picker is retired** — the source language is now auto-detected by OCR and merely *confirmed* in OCRReview (CHANGE 1), so no screen forces a pre-pick. What survives, on the **Library entry only**, is an **optional pre-capture "Page language" hint** (default **Auto-detect**, over the full `LanguageCatalog` set): if the user already knows the page's language they can bias Vision before OCR, but leaving it on Auto-detect is the default (DECISIONS #25, OCR_PIPELINE §1/§2).
...
```
Library (book == nil):
  capture/import ▶ OCR(auto-detect · optional pre-capture Page-language hint) ▶ OCRReview(edit · confirm source · choose translate-to)
...
New screen shown **after OCR, before persistence** (CHANGE 2) — editing here is free because nothing is saved yet, so no srs/bookmark is at risk. It prefills a full-height editable `TextEditor` with `OCRResult.text` and a source-language `Picker` over the **full recognizable set** (`LanguageCatalog.options` — every Vision `supportedRecognitionLanguages` code, not a curated nine; DECISIONS #25), prefilled with `OCRResult.detectedLanguageCode` (the correctable path for a wrong detection; see [OCR_PIPELINE.md](OCR_PIPELINE.md) for how detection is computed via `NLLanguageRecognizer`). An optional translate-to `Picker` (seeded from the **native language** when translation is on, or inherited from `book.translationLanguage` on the Add-Page path; includes "None") sets the book's target — subsystem detail in [TRANSLATION_DESIGN.md](TRANSLATION_DESIGN.md) §7.
...
- Language source in persisted mode: `page.book?.languageCode` — the **source** language, auto-detected and stored per Book (DECISIONS #25). `ScanPage.book` is Optional in the schema, but every route is built from a page listed under a Book, so a bare defensive constant covers the impossible-nil case; **not** an `@AppStorage("targetLanguage")` setting — there is no source-language setting anymore (the per-user setting is the *native* language, a translation destination, never the TTS source). Never force-unwrap. The same resolved code feeds `SaveWordSheet` (§7).

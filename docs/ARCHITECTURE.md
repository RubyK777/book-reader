...
`Book (title, languageCode, translationLanguage?) 1─* ScanPage (rawText, orderIndex, audioData?) 1─* Sentence (text, translatedText?, orderIndex, isBookmarked, userNote, srs) 1─* Annotation (type, text, contextSentence, languageCode, srs, …)`. Saved words, phrases, sentences, and grammar points are all **`Annotation`s** typed by `AnnotationType` — the legacy standalone `SavedWord` model was folded into `Annotation` in **schema V5** (DECISIONS #63). `SRSState` is a Codable **value type** (SM-2) embedded in both Sentence and Annotation.
...
2. *(Resolved by DECISIONS #63.)* Saved words are now `Annotation`s linked to their `Sentence` (`annotation.sentence?.page?.book`), so a saved word records its source — the old standalone `SavedWord` (which didn't) is gone.

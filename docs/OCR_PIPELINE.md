...
**Pipeline is capture-first.** The user photographs a page in any *supported* language (§2's honest-scope note) **without pre-picking one**; Vision auto-detects the script, `NLLanguageRecognizer` names the language, and the user confirms/corrects it in **OCRReview** (§4.5) before anything persists. The source language is a property of the **Book/page, not the user** (DECISIONS #25) — the per-user setting is the *native* language (§4.5, PHASE3 §4), not this. An **optional pre-capture "Page language" hint** (default **Auto-detect**) is offered on the **Library entry only**: choosing a language biases Vision's `recognitionLanguages` before OCR when the user already knows the page's language; leaving it on Auto-detect is the default path. Canonical flow (used verbatim across docs):

- **Library entry (book unknown):** capture/import → OCR(auto-detect, or the optional pre-capture Page-language hint) → OCRReview(edit text · confirm source language · choose translate-to) → assign (pick or quick-create Book; source language pre-filled from detection) → persist → push Reader
...
- **`automaticallyDetectsLanguage = true`** (iOS 16+, well within the 18.0 floor). Capture-first means we do *not* know the language up front, so we let Vision pick the recognition model. There are **two** ways a hint arrives: (a) the **Add Page** path (§1), where the page joins a Book that already has a `languageCode` and the caller passes it as `languageHint`; and (b) the **optional pre-capture Page-language hint** on the Library entry, where the user pre-selects a language from `LanguageCatalog`. In either case we set `recognitionLanguages = [hint]` (full BCP-47; the trim-at-boundary rule is NL's, not Vision's) instead of `automaticallyDetectsLanguage`, pinning the known-language model to improve correction on short/noisy lines. Trade-off: a hint-less first scan gives up the known-language prior; recovered by the pre-capture hint, by the user confirming/correcting the language in OCRReview (§4.5), and, for stubborn pages, a hint-driven re-read from there.
...
- **Honest scope of "any language."** Auto-detect (and the pre-capture / OCRReview language options) ranges over `VNRecognizeTextRequest.supportedRecognitionLanguages(for: .accurate, …)`, surfaced app-wide via **`LanguageCatalog`** (`Shared/Languages.swift`, the source-language option set that replaced the old 9-item `SupportedLanguage` enum — DECISIONS #25) — Latin-script languages plus zh/ja/ko/…, a **bounded list, not literally every language**, but far broader than the retired nine. This is one of **three separately-bounded sets**: recognizing a language (this set) is distinct from *hearing* it (installed `AVSpeechSynthesisVoice`) and *translating* it (`LanguageAvailability`), so a page can OCR fine in a language with no voice — surface that gap downstream, don't shrink this set to match. A page in an unsupported script yields garbage/empty text and trips the §4 gate; state this limit plainly in any "any language" capability copy.
...
- a **source-language** `Picker` prefilled with `detectedLanguageCode` and offering the **full recognizable set** (`LanguageCatalog.options`, i.e. every `supportedRecognitionLanguages` code — not a curated nine) — this is how the user overrides a wrong `NLLanguageRecognizer` guess. The confirmed code is what `SentenceSplitter` uses and what seeds `Book.languageCode` on a book's first page. A **Re-read with this language** button re-runs `recognizeText(languageHint:)` on the retained image when detection was off (§4).
...
7. *(Resolved — DECISIONS #25.)* OCRReview's source-language picker lists the `supportedRecognitionLanguages` set (via `LanguageCatalog`), so **Re-read** is always possible for anything the user can pick. Residual: whether to *additionally* allow labeling an unsupported-script page purely for splitting/TTS (where re-OCR can't help) — deferred; the recognition set is the pragmatic choice for v1.

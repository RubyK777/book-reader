...
Test: launch with `["-uiTestFixture=fixture-fr-page", "-ocrLanguageHint", "fr-FR"]`. Under capture-first auto-detect there is no source-language `@AppStorage` key to pin (that was the retired `targetLanguage`); instead the DEBUG fixture hook forwards `-ocrLanguageHint` into `OCRService.recognizeText(languageHint:)` so the French fixture is recognized deterministically rather than depending on auto-detection landing on `fr` under test. (Omitting the hint exercises the auto-detect path but is flakier.) Wait for the Reader (`app.navigationBars["Reader"]`, timeout 15 s — covers the ≤ 10 s budget plus slack), assert ≥ 1 sentence card exists and the play button is hittable.
...
- [ ] Add fixture page photos to `Fixtures/` and one to `DebugAssets.xcassets` excluded from Release in `project.yml` (§3); add Reader accessibility identifiers (`sentence-card-\(index)`, `play-pause`) + labels on icon-only playback buttons; implement the `-uiTestFixture` + `-ocrLanguageHint` launch hooks + XCUITest smoke — *acceptance: test launches with `-ocrLanguageHint fr-FR`, reaches Reader, finds ≥ 1 sentence card, in ≤ 15 s; Release archive contains no fixture image.*

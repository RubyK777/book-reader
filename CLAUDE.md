...
- **Embedded Codable structs (`SRSState`, `LearningAssets`) are part of the schema fingerprint** — changing one (even adding an optional field) requires freezing the previous schema version and adding a new lightweight stage, exactly like a @Model change (DECISIONS #35).
